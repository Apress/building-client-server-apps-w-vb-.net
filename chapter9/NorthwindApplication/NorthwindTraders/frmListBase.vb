...
#End Region
    ...
    Public Sub Find() Implements IFind.Find
        Try
            Dim i As Integer
    ...
    Public Sub FindNext() Implements IFind.FindNext
        Dim i, j, k As Integer
        Dim lst As ListViewItem

        If lvwList.SelectedItems.Count = 0 Then Exit Sub

        Try
            i = cboColumn.SelectedIndex
            k = lvwList.SelectedItems(0).Index + 1

            For j = k To lvwList.Items.Count - 1
                lst = lvwList.Items(j)
                If lst.SubItems(i).Text.ToUpper.StartsWith(txtSearch.Text.ToUpper) Then
                    lst.Selected = True
                    lst.EnsureVisible()
